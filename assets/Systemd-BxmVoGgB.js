import{j as e,T as a}from"./index-_kFPLDpE.js";import{P as r}from"./PageContainer-eafNNqkI.js";import{C as s}from"./CodeBlock-ByIkwE54.js";import{A as o}from"./AlertBox-NzOB7YP7.js";function c(){return e.jsxs(r,{title:"Systemd e Serviços",subtitle:"Gerenciando serviços, timers, logs e o processo de boot no Ubuntu com systemd e systemctl.",difficulty:"intermediario",timeToRead:"25 min",children:[e.jsxs(o,{type:"info",title:"Pré-requisitos",children:["Ubuntu 16.04+ (qualquer versão moderna usa systemd). Acesso ",e.jsx("code",{children:"sudo"}),". Conhecer terminal básico."]}),e.jsx("h2",{children:"Glossário rápido"}),e.jsxs("p",{children:[e.jsx("strong",{children:"systemd"})," — sistema de init que substituiu o SysVinit. Cuida de inicialização, serviços, logs (journald), timers, sockets, montagens."]}),e.jsxs("p",{children:[e.jsx("strong",{children:"Unit"}),' — qualquer "coisa" gerenciada pelo systemd. Tipos:',e.jsx("code",{children:".service"}),", ",e.jsx("code",{children:".timer"}),", ",e.jsx("code",{children:".socket"}),", ",e.jsx("code",{children:".mount"}),",",e.jsx("code",{children:".target"}),", ",e.jsx("code",{children:".path"}),"."]}),e.jsxs("p",{children:[e.jsx("strong",{children:"Target"})," — agrupamento de units que define um estado do sistema.",e.jsx("code",{children:"multi-user.target"})," = modo servidor; ",e.jsx("code",{children:"graphical.target"})," = com GUI."]}),e.jsxs("p",{children:[e.jsx("strong",{children:"Journal"})," — log binário centralizado mantido pelo ",e.jsx("code",{children:"journald"}),". Lê-se com ",e.jsx("code",{children:"journalctl"}),"."]}),e.jsxs("p",{children:["O ",e.jsx("strong",{children:"systemd"})," é o sistema de inicialização (init system) e gerenciador de serviços do Ubuntu (desde o Ubuntu 15.04). Ele substitui o antigo SysVinit e é responsável por iniciar todos os serviços do sistema — do NetworkManager ao servidor SSH, do cron ao servidor de impressão."]}),e.jsx("h2",{children:"systemctl: O Comando Principal"}),e.jsx("h3",{children:"Gerenciando Serviços"}),e.jsx(s,{title:"Operações básicas com serviços",code:`# Verificar status de um serviço (mostra se está ativo, com falha, logs recentes)
...
# Saída: active | inactive | failed`}),e.jsx("h3",{children:"Listando Serviços"}),e.jsx(s,{title:"Listar e filtrar serviços com flags",code:`# Listar todos os serviços ATIVOS no momento
...
systemctl --failed
# --failed = equivalente a --state=failed (atalho conveniente)`}),e.jsxs(o,{type:"info",title:"Diferença entre enable e start",children:[e.jsx("code",{children:"start"})," = iniciar o serviço AGORA (até o próximo reboot).",e.jsx("br",{}),e.jsx("code",{children:"enable"})," = configurar para iniciar NO BOOT (não inicia agora).",e.jsx("br",{}),e.jsx("code",{children:"enable --now"})," = faz os dois ao mesmo tempo. É o mais usado na prática."]}),e.jsx("p",{children:"Um serviço que falha ao iniciar — e como o systemd te mostra isso:"}),e.jsx(a,{title:"wallyson@ubuntu: ~",lines:[{type:"cmd",text:"systemctl start nginx"},{type:"err",text:"Failed to start nginx.service: Access denied"},{type:"warn",text:"-> operacoes que mudam estado precisam de sudo:"},{type:"cmd",text:"sudo systemctl start nginx"},{type:"err",text:"Job for nginx.service failed because the control process exited with error code."},{type:"err",text:"See 'systemctl status nginx.service' and 'journalctl -xeu nginx.service'."},{type:"cmd",text:"systemctl status nginx.service"},{type:"out",text:"  nginx.service - A high performance web server and reverse proxy"},{type:"err",text:"     Active: failed (Result: exit-code) since Sat 2026-07-04 20:31:02"},{type:"out",text:"  nginx: [emerg] bind() to 0.0.0.0:80 failed (98: Address already in use)"},{type:"warn",text:"-> a porta 80 ja esta ocupada (ex: apache rodando). Pare o outro servico."}]}),e.jsx("h2",{children:"journalctl: Lendo os Logs do Sistema"}),e.jsxs("p",{children:["O systemd centraliza todos os logs do sistema no ",e.jsx("strong",{children:"journal"}),". O comando",e.jsx("code",{children:"journalctl"})," é a ferramenta para ler e filtrar esses logs — muito mais poderoso que abrir arquivos de log manualmente."]}),e.jsx(s,{title:"journalctl com todas as flags explicadas",code:`# Ver todos os logs (do mais antigo ao mais novo — pressione q para sair)
journalctl
...
journalctl --disk-usage`}),e.jsx("h2",{children:"Anatomia de um Arquivo de Unit"}),e.jsxs("p",{children:["Os serviços do systemd são configurados via arquivos chamados ",e.jsx("strong",{children:"units"}),". Eles ficam em:"]}),e.jsxs("ul",{children:[e.jsxs("li",{children:[e.jsx("code",{children:"/lib/systemd/system/"})," — Units do sistema (instaladas por pacotes)"]}),e.jsxs("li",{children:[e.jsx("code",{children:"/etc/systemd/system/"})," — Units personalizadas (têm prioridade maior)"]}),e.jsxs("li",{children:[e.jsx("code",{children:"~/.config/systemd/user/"})," — Units do usuário (sem root)"]})]}),e.jsx(s,{title:"Estrutura de um arquivo .service",code:`# Ver o arquivo de um serviço existente:
...
WantedBy=multi-user.target`}),e.jsx("h3",{children:"Criando um Serviço Personalizado"}),e.jsx(s,{title:"Criar um serviço systemd customizado",code:`# Exemplo: rodar um script Python como serviço do sistema
...
journalctl -u meu-app -f`}),e.jsx("h2",{children:"Systemd Timers (Substituto do Cron)"}),e.jsx("p",{children:"Timers do systemd são mais poderosos que o cron clássico — têm logging integrado, suporte a dependências, e são mais fáceis de depurar."}),e.jsx(s,{title:"Criar um timer systemd",code:`# Timer que roda um script de backup todos os dias às 3h da manhã
...
systemctl status backup-diario.timer`}),e.jsx("h2",{children:"Targets (Equivalente aos Runlevels)"}),e.jsx(s,{title:"Targets: modos de operação do sistema",code:`# Ver o target padrão (modo em que o sistema inicia)
...
sudo systemctl hibernate`}),e.jsx("h2",{children:"Analisando o Tempo de Boot"}),e.jsx(s,{title:"Diagnóstico de velocidade de boot",code:`# Ver o tempo total do boot e quanto foi kernel vs userspace
systemd-analyze
# Saída:
...
firefox boot-plot.svg`})]})}export{c as default};
